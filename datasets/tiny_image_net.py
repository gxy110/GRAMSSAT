...
class TinyImageNetSetup(DatasetSetup):
    ...
    def get_transformed_labeled_dataset(self, n_labeled_per_class, file_path):
        transforms_ = self.get_transforms()
        base_dataset = TinyImageNet(file_path)
        train_labeled_idxs, _ = train_val_split(list(base_dataset.labels.values()), n_labeled_per_class, self.num_classes)
        train_labeled_dataset = TinyImageNetLabeled(file_path, train_labeled_idxs, split='train', transform=transforms_)
