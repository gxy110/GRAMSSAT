...
class CustomRandomSampler(Sampler):
    def __init__(self, data_source, seed_callback):
    ...
    def __iter__(self):
        ...
        g = torch.Generator()
        g.manual_seed(self.seed_callback())  
...
class SpecificIndexSampler(Sampler):
    def __init__(self, data_source, indices, batch_size):
        self.data_source = data_source
        self.indices = indices
        self.batch_size = batch_size

    def __iter__(self):
        # Start with the specified indices
        yield from self.indices
        # Then yield the rest of the indices, ensuring no repetition with the ones already chosen
        rest_indices = [i for i in range(len(self.data_source)) if i not in self.indices]
        yield from rest_indices[:self.batch_size-len(self.indices)]
        yield from rest_indices[self.batch_size-len(self.indices):]
...
def get_seed_for_current_epoch(current_epoch):
    seed_for_epoch = 42 + current_epoch  
    return seed_for_epoch
